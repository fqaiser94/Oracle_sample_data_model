...
string <- c("INSERT INTO orders VALUES (2458
...
#                replacement="CAST(AS DATETIME)"
#                )

# string <- gsub(x=string, 
#                pattern="TO_TIMESTAMP\\((\\'^.*\\'$),(\\'^.*\\'$),(\\'^.*\\'$)\\)", 
#                replacement="CAST(\\2 AS DATETIME)"
# )
# 
# 
# string
...
string = data.frame(og = unlist(string), stringsAsFactors = FALSE) %>%
  mutate(time_raw = str_extract(og, pattern = "TO_TIMESTAMP\\([^\\)]+\\)")) %>%
  mutate(# extract datetime portion
         temp = str_extract(time_raw, pattern = "'[^\\)]+"),
         # extract clean date
         date = gsub(x=temp, pattern=" .*$", replacement=''),
         date = sub(x=date, pattern=".", replacement=''),
         # extract clean time
         time = gsub(x=temp, pattern=".*? (.+)", replacement = "\\1"),
         time = gsub(x=time, pattern=" .*$", replacement = ""),
         time = gsub(x=time, pattern="[^\\.]*$", replacement = ""),
         time = gsub(x=time, pattern='.{1}$', replacement = ''),
         time = gsub(x=time, pattern='\\.', replacement = ':'),
         # clean datetime
         datetime = paste(date, time, sep=' ')
         ) %>%
  select(og, time_raw, datetime) %>%
  mutate(
    # replacement string
    rep_str = paste0(" CAST('",  datetime, "' AS DATETIME)"),
    # replace 
    final = str_replace_all(string = og, pattern = "TO_TIMESTAMP\\([^\\)]+\\)", replacement=rep_str)
  ) %>%
  select(final)


write.csv(x = string$final, file = '/home/fqaiser94/Desktop/temp.csv', quote = FALSE, row.names=FALSE)