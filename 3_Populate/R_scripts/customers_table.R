...
string <- gsub(x=string, 
               pattern='cust_address_typ\n            \\([^\\)]+\\)', 
               replacement="'ADDRESS'"
               )

string <- gsub(x=string, 
               pattern='PHONE_LIST_TYP\n            \\([^\\)]+\\)', 
               replacement="'PHONE NUM'"
)


string <- gsub(x=string, 
               pattern='MDSYS.SDO_GEOMETRY\n            \\([^\\)]+\\)', 
...
string <- gsub(x=string, 
               pattern='149,NULL\\)', 
               replacement="149,NULL,NULL,NULL,NULL,NULL\\)"
)
...
write.csv(x = string, file = 'Desktop/temp.csv', quote=FALSE, row.names=FALSE)